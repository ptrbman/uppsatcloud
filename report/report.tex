...
during various intervals over the course of one day. The runtime was
measured for each execution (the underlying software is
deterministic) and some key numbers are presented in Table~\ref{tbl:one}.
...
\begin{tabular}{|l|c|}
  \hline
  Average runtime &  13.34 \\
  \hline
  Maximum runtime & 16.14 \\
  \hline
  Minimum runtime & 12.48 \\
  \hline      
...
\begin{Figure}
  ...
  \hline
  Maximum standard deviation runtime & 0.95 & 0.39 \\
  \hline
\end{tabular}
\captionof{table}{Statistics of error measurements (in seconds).}\label{tbl:one}
\end{Figure}
...
% Det går att bryta ut hela Celery-cirkusen om den behöver gå fortare
% Workers osv skalar i princip obegränsat
% Front-enden är JÄTTESNABB, men om man MÅSTE kan man ju ha en load balancer osv
% (i princip kan flera front-ends köras HELT OBEROENDE eftersom de bara skickar
% jobb till kön)
% Workers är också oberoende så det är bara att hälla på fler
...
% Mer funktioner! Bells and whistles! Stöd för att köra experiment och
% rapportera felmarginalen! Load balancer för deployments som skalar upp när kön
% är full! Autenticering!
% 
% Continuous Integration med Travis?
